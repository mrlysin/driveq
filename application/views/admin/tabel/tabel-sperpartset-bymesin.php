<div class="table-responsive">
  <table id="tabeldetailbefore" class="table table-striped table-bordered first">
    <thead>
      <tr>
        <th>Foto</th>
        <th>Kode</th>
      ...
      <form id="tambahsperpart">
        <div class="modal-body">
          <div class="form-group">
            <label>Jumlah</label>
            <input type="hidden" name="mesinset" id="req_id" value="<?php echo $req_id; ?>">
...
<script>
  ...
  $("form#tambahsperpart").submit(function(e) {
    ...
    $.ajax({
      url: "<?php echo base_url('api/prosses/inputdetailmesinset') ?>",
      type: 'POST',
      dataType: 'Json',
      data: formData,
      contentType: false,
      ...
      success: function(data) {
        if (data['Status'] == 'Sukses') {
          ...
          $(".infobefore").fadeTo(2000, 500).slideUp(500, function() {
            $(".infobefore").slideUp(500);
          });
          $('.infobefore').html('<div class="alert alert-success" role="alert">' + data['Notif'] + '</div>');
          setTimeout(function() {
            $(".detail").load("<?php echo base_url('admin/tabelmesinsetdetail/') ?>" + $('#req_id').val());
            $(".tabelsperpart").load("<?php echo base_url('admin/sperpartsetbymesin/') ?>" + $('#req_id').val()+"/"+$('#req_id').val());
          }, 1000);
        } else {
          $(".infobefore").fadeTo(2000, 500).slideUp(500, function() {
            $(".infobefore").slideUp(500);
          });
          $('.infobefore').html('<div class="alert alert-danger" role="alert">' + data['Notif'] + '</div>');
